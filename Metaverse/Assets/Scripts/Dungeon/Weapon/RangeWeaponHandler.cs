...
// Quarternion은 수학적으로 회전을 나타내는 4차원 수이기 때문에 일반적으로는 벡터에 곱할 수 없다.
// 하지만 유니티에서는 벡터를 Quarternion이 가진 회전 만큼 회전 시킨 결과를 반환하는 연산자가 존재하기 
// 때문에 순서를 맞춰 입력하면 연산이 동작한다.
// 반드시 왼쪽에 Quarternion, 오른쪽에 벡터를 놓아야한다.